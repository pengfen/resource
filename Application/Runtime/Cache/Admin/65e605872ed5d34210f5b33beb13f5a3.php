...
<div id="add">
    <a href="javascript:void(0)">添加会员</a>
    <input type='hidden' value="<?php echo U('User/add');?>"/>
	...
	<table class="table table-striped">
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
	    		...
	    		<td><?php if($v[sex] == 0): ?>保密<?php elseif($v[sex] == 1): ?>男<?php else: ?>女<?php endif; ?></td>
	    		<td><?php echo ($v["email"]); ?></td>
	    		<td><?php if($v[state] == 0): ?>禁用<?php else: ?>启用<?php endif; ?></td>
	    		<td>
	    			<a class="update" href="javascript:void(0)">修改</a>
	    			<input type="hidden" name="" value="<?php echo U('User/edit', array('id'=>$v[id], 'state'=>$v['state'], 'name'=>$v[username]));?>" />
	    			<a class="upd" href="<?php echo U('LogCate/index', array('id'=>$v[id], 'name'=>$v[username]));?>">日志分类</a>
	    			<a class="album" href="<?php echo U('Album/index', array('id'=>$v[id], 'name'=>$v[username]));?>">相册分类</a>
	    			<a class="detail" href="javascript:void(0)">会员详情</a>
	    			<input type="hidden" value="<?php echo U('User/detail', array('id'=>$v[id]));?>"/>
	    			<a href="<?php echo U('Mood/index', array('id'=>$v[id], 'name'=>$v[username]));?>">说说列表</a>
	    			<a href="<?php echo U('Friend/index', array('id'=>$v[id], 'name'=>$v[username]));?>">好友列表</a>
	    			<a href="<?php echo U('Record/index', array('id'=>$v[id], 'name'=>$v[username]));?>">生活记录列表</a>
	    			<a href="<?php echo U('Advice/index', array('id'=>$v[id], 'name'=>$v[username]));?>">建议列表</a>
	    		</td>
