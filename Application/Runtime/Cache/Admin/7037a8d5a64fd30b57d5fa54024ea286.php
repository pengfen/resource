...
<div id="add">
    ...
    <input type='hidden' value="<?php echo U('Auth/add');?>"/>
    <a href="<?php echo U('Auth/index');?>">返回权限列表</a>
</div>
	...
	<table class="table table-striped">
	    ...
	    <tbody>
	    	<?php if(is_array($info)): foreach($info as $key=>$v): ?><tr>
	    		<td><?php echo ($v["id"]); ?></td>
	    		<td><a class="detail" href="javascript:void(0);"><?php echo ($v["name"]); ?></a>
	    		    <input type="hidden" value="<?php echo U('Auth/detail', array('id'=>$v[id]));?>"/>
	    		</td>
	    		...
	    		<td>
	    			...
	    			<a class="upd" href="javascript:void(0);">修改</a>
				  	<input type="hidden" value="<?php echo U('Auth/edit', array('id'=>$v[id]));?>"/>
				  	<a class="upd-img" href="javascript:void(0);">修改关联图</a>
				  	<input type="hidden" value="<?php echo U('Auth/editImg', array('id'=>$v[id]));?>"/>
				  	<?php if($v[id] > 2): ?><!-- 入口权限管理和权限列表都不能删除 -->
				  	<a href="<?php echo U('Auth/delete', array('id'=>$v[id]));?>">删除</a><?php endif; ?>
	    		</td>
...
<script type="text/javascript">
	...
	click($('.upd-img')); // 动态加载修改关联图模板
	click($('.detail')); // 动态加载详情界面模板
</script>
