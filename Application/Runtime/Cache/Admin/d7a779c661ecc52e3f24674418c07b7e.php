<?php if (!defined('THINK_PATH')) exit();?><li class="green" id="message">
...
var $url = "<?php echo U('Index/message');?>";
var setting = {
	url:$url,
	dataType:'json',
	success:function(data){
		...
		if ($len) {
		    ...
		    for (var i = 0; i < $len; i ++) {
				...
				$id = data[i].id;
				//console.log($id);
				$url = "/resource/index.php/Admin/Advice/detail/id/" + parseInt($id);
				//console.log($url);
				$title = data[i].title;
				// $date = new Date();
			...
			click($('.send-message'));
			$url = "/resource/index.php/Admin/Advice/read/flag/" + parseInt(1); 
